...
const AssessmentPanel: React.FC = () => {
  ...
  const renderQuestion = () => {
    ...
    const bloomLabel = currentQuestion.bloomLabel
      ? `${currentQuestion.bloomLabel}`
      : typeof currentQuestion.bloomLevel === 'number'
        ? `L${currentQuestion.bloomLevel}`
        : currentQuestion.bloomLevel
          ? String(currentQuestion.bloomLevel)
          : null;
    const bloomTag = bloomLabel ? (
      <span className={`inline-flex px-2 py-1 text-[11px] rounded-full ${
        isDarkMode ? 'bg-indigo-900/60 text-indigo-300' : 'bg-indigo-100 text-indigo-700'
      }`}>
        Bloom {bloomLabel}
      </span>
    ) : null;
    const wrapWithBloom = (node: React.ReactNode) => (

...
exports.recalculateAnalytics = exports.testOrgWebhook = exports.genieEvidenceExport = exports.notificationDispatcher = exports.assessmentResultCreatedOrg = exports.assessmentResultCreated = exports.enrollmentCompletionIssuerOrg = exports.enrollmentCompletionIssuer = exports.assessmentResultGuardOrg = exports.assessmentResultGuard = exports.enrollmentStatusGuardOrg = exports.enrollmentStatusGuard = exports.retentionPolicyScheduler = exports.archiveOldCoursesScheduler = exports.analyticsRefreshScheduler = exports.competencyRefreshScheduler = exports.policyUpdated = exports.coursePublishedOrg = exports.coursePublished = exports.memberCreated = exports.enrollmentCreatedOrg = exports.enrollmentCreated = exports.checkOverdueEnrollments = exports.sendDeadlineReminders = exports.managerDigestProcessor = exports.managerDigestScheduler = exports.genieReportScheduler = exports.genieTranscribeAudio = exports.genieTextToSpeech = exports.genieWebSearch = exports.genieChatCompletion = exports.genieGenerateLessonCritique = exports.genieGenerateLessonContent = exports.genieGenerateOutline = exports.genieGenerateObjectives = exports.apiV1 = void 0;
...
const getOpenAI = () => {
    ...
    if (!apiKey) {
        throw new https_1.HttpsError('failed-precondition', 'OpenAI API key is not configured on the server.');
    }
    return new openai_1.default({ apiKey });
};
const toCallableAiError = (error, fallbackMessage) => {
    var _a, _b;
    if (error instanceof https_1.HttpsError) {
        return error;
    }
    const status = Number((_b = (_a = error === null || error === void 0 ? void 0 : error.status) !== null && _a !== void 0 ? _a : error === null || error === void 0 ? void 0 : error.statusCode) !== null && _b !== void 0 ? _b : 0);
    const rawMessage = (error === null || error === void 0 ? void 0 : error.message) || fallbackMessage;
    const message = rawMessage.toLowerCase();
    if (status === 400 && (message.includes('maximum context length')
        || message.includes('context_length_exceeded')
        || message.includes('too many tokens'))) {
        return new https_1.HttpsError('invalid-argument', 'Your request is too long for the AI model. Try starting a new chat or shortening your message.');
    }
    if (status === 401 || message.includes('api key') || message.includes('not configured')) {
        return new https_1.HttpsError('failed-precondition', 'AI service is not configured correctly on the server.');
    }
    if (status === 429 || message.includes('rate limit') || message.includes('quota')) {
        return new https_1.HttpsError('resource-exhausted', 'AI service is busy right now. Please try again shortly.');
    }
    if (status === 0 || message.includes('connection error') || message.includes('network error')) {
        return new https_1.HttpsError('unavailable', 'Server cannot reach OpenAI right now. Check Firebase billing plan (Blaze) and function outbound internet access.');
    }
    if (status >= 500 || message.includes('timeout') || message.includes('timed out')) {
        return new https_1.HttpsError('unavailable', 'AI service is temporarily unavailable. Please try again.');
    }
    const safeDetails = {
        status,
        message: rawMessage.slice(0, 500)
    };
    return new https_1.HttpsError('internal', fallbackMessage, safeDetails);
};
...
const resolveDuckDuckGoUrl = (rawUrl) => {
...
const parseDuckDuckGoHtml = (html) => {
    ...
    while ((match = anchorRegex.exec(html)) && results.length < 8) {
        ...
        const snippet = stripHtml(decodeHtmlEntities((snippetMatch === null || snippetMatch === void 0 ? void 0 : snippetMatch[1]) || '')).trim() || 'No description available';
        let source = '';
        try {
...
const fetchDuckDuckGoWebSearch = async (query) => {
...
const logAiUsage = async (orgId, action, model, usage) => {
    if (!orgId || !usage)
        return;
    const entry = {
        orgId,
...
const createNotification = async (payload) => {
    ...
    const webhook = orgSnap.exists ? (_c = (_b = (_a = orgSnap.data()) === null || _a === void 0 ? void 0 : _a.settings) === null || _b === void 0 ? void 0 : _b.notifications) === null || _c === void 0 ? void 0 : _c.webhookUrl : undefined;
...
const startAnalyticsJob = async (orgId, mode) => {
...
const completeAnalyticsJob = async (orgId, jobId, status, errorMessage) => {
    const update = {
        status,
...
const getOrgApiKey = async (orgId) => {
    var _a, _b;
    const orgSnap = await db.collection('organizations').doc(orgId).get();
    if (!orgSnap.exists)
        return null;
    return ((_b = (_a = orgSnap.data()) === null || _a === void 0 ? void 0 : _a.settings) === null || _b === void 0 ? void 0 : _b.apiKey) || null;
};
const authorizeRequest = async (req) => {
    const orgId = String(req.headers['x-org-id'] || req.query.orgId || '');
    if (!orgId)
        return { ok: false, orgId: null, reason: 'orgId required' };
...
const dispatchWebhooks = async (orgId, event, data) => {
    ...
    for (const hookDoc of hooksSnap.docs) {
        ...
        if (!hook.url)
            continue;
        if (Array.isArray(hook.events) && hook.events.length > 0 && !hook.events.includes(event))
            continue;
        try {
...
api.post('/api/v1/courses/generate', async (req, res) => {
    ...
    try {
        ...
        res.json({ content: ((_b = (_a = response.choices[0]) === null || _a === void 0 ? void 0 : _a.message) === null || _b === void 0 ? void 0 : _b.content) || '' });
    }
    catch (error) {
        await logSystemError({
            orgId,
...
api.post('/api/v1/integrations/sso', async (req, res) => {
    ...
    const record = {
        ...
        provider,
        config,
        enabled: Boolean((_a = body.enabled) !== null && _a !== void 0 ? _a : true),
        createdAt: Date.now(),
        updatedAt: Date.now()
    };
...
exports.apiV1 = (0, https_1.onRequest)(api);
...
exports.genieGenerateObjectives = (0, https_1.onCall)({ secrets: [openAIKey] }, async (request) => {
    var _a, _b;
    requireAuth(request.auth);
    const openai = getOpenAI();
    const { sources, context, count, orgId } = request.data;
    if (!sources || sources.length === 0) {
        throw new https_1.HttpsError('invalid-argument', 'Sources are required.');
    }
    const sourceSummary = sources.map((source, index) => (`${index + 1}. ${source.title} (${source.type})${source.description ? ` - ${source.description}` : ''} [tags: ${source.tags.join(', ') || 'none'}]`)).join('\n');
...
Return JSON only with this shape:
{
  "objectives": ["Objective 1", "Objective 2"]
    ...
    if (!content) {
        throw new https_1.HttpsError('internal', 'No objectives returned.');
    }
    const parsed = parseJsonFromContent(content);
    if (!parsed.objectives || !Array.isArray(parsed.objectives)) {
        throw new https_1.HttpsError('internal', 'Invalid objectives format.');
    }
    await logAiUsage(orgId, 'genieGenerateObjectives', model, response.usage);
    return { objectives: parsed.objectives };
});
exports.genieGenerateOutline = (0, https_1.onCall)({ secrets: [openAIKey] }, async (request) => {
    var _a, _b;
    requireAuth(request.auth);
    const openai = getOpenAI();
    const { title, sources, prompt, orgId } = request.data;
    if (!title || !sources || sources.length === 0) {
        throw new https_1.HttpsError('invalid-argument', 'Title and sources are required.');
    }
    const sourceSummary = sources.map((source, index) => (`${index + 1}. ${source.title} (${source.type})${source.description ? ` - ${source.description}` : ''} [tags: ${source.tags.join(', ') || 'none'}]`)).join('\n');
...
Return JSON only with this shape:
{
  "modules": [
    ...
    const content = (_b = (_a = response.choices[0]) === null || _a === void 0 ? void 0 : _a.message) === null || _b === void 0 ? void 0 : _b.content;
    if (!content) {
        throw new https_1.HttpsError('internal', 'No outline returned.');
    }
    const parsed = parseJsonFromContent(content);
    if (!parsed.modules || !Array.isArray(parsed.modules)) {
        throw new https_1.HttpsError('internal', 'Invalid outline format.');
    }
    await logAiUsage(orgId, 'genieGenerateOutline', model, response.usage);
    return { modules: parsed.modules };
});
exports.genieGenerateLessonContent = (0, https_1.onCall)({ secrets: [openAIKey] }, async (request) => {
    var _a, _b;
    requireAuth(request.auth);
    const openai = getOpenAI();
    const { courseTitle, moduleTitle, lessonTitle, lessonType, sources, prompt, orgId } = request.data;
    if (!courseTitle || !moduleTitle || !lessonTitle || !lessonType) {
        throw new https_1.HttpsError('invalid-argument', 'Course, module, lesson, and type are required.');
    }
    const sourceSummary = sources.map((source, index) => (`${index + 1}. ${source.title} (${source.type})${source.description ? ` - ${source.description}` : ''} [tags: ${source.tags.join(', ') || 'none'}]`)).join('\n');
...
Return JSON only.`;
    ...
    const content = (_b = (_a = response.choices[0]) === null || _a === void 0 ? void 0 : _a.message) === null || _b === void 0 ? void 0 : _b.content;
    if (!content) {
        throw new https_1.HttpsError('internal', 'No lesson content returned.');
    }
    const parsed = parseJsonFromContent(content);
    if (!parsed.content) {
        throw new https_1.HttpsError('internal', 'Invalid lesson content format.');
    }
    await logAiUsage(orgId, 'genieGenerateLessonContent', model, response.usage);
    return parsed;
});
exports.genieGenerateLessonCritique = (0, https_1.onCall)({ secrets: [openAIKey] }, async (request) => {
    var _a, _b;
    requireAuth(request.auth);
    const openai = getOpenAI();
    const { courseTitle, moduleTitle, lessonTitle, lessonType, lessonContent, sources, prompt } = request.data;
    if (!courseTitle || !moduleTitle || !lessonTitle || !lessonType || !lessonContent) {
        throw new https_1.HttpsError('invalid-argument', 'Lesson details and content are required.');
    }
    const sourceSummary = sources.map((source, index) => (`${index + 1}. ${source.title} (${source.type})${source.description ? ` - ${source.description}` : ''} [tags: ${source.tags.join(', ') || 'none'}]`)).join('\n');
...
`;
    ...
    return { critique: ((_b = (_a = response.choices[0]) === null || _a === void 0 ? void 0 : _a.message) === null || _b === void 0 ? void 0 : _b.content) || 'No critique returned.' };
});
exports.genieChatCompletion = (0, https_1.onCall)({ secrets: [openAIKey] }, async (request) => {
    var _a, _b;
    try {
        requireAuth(request.auth);
        const openai = getOpenAI();
        const { model, systemPrompt, userPrompt, temperature, maxTokens, responseFormat, orgId } = request.data;
        if (!systemPrompt || !userPrompt) {
            throw new https_1.HttpsError('invalid-argument', 'systemPrompt and userPrompt are required.');
        }
        ...
        for (const candidate of modelCandidates) {
            try {
                ...
                response = await openai.chat.completions.create({
                    ...
                    max_tokens: maxTokens !== null && maxTokens !== void 0 ? maxTokens : 1200,
        ...
        if (!response) {
            throw lastModelError !== null && lastModelError !== void 0 ? lastModelError : new Error('No AI model available for chat completion.');
        }
        const content = (_b = (_a = response.choices[0]) === null || _a === void 0 ? void 0 : _a.message) === null || _b === void 0 ? void 0 : _b.content;
        if (!content) {
            throw new https_1.HttpsError('internal', 'No completion returned.');
        }
...
exports.genieWebSearch = (0, https_1.onCall)(async (request) => {
    try {
        requireAuth(request.auth);
        ...
        if (message.toLowerCase().includes('abort')) {
            throw new https_1.HttpsError('deadline-exceeded', 'Web search timed out. Please try again.');
        }
        throw new https_1.HttpsError('unavailable', 'Web search is temporarily unavailable.');
    }
});
exports.genieTextToSpeech = (0, https_1.onCall)({ secrets: [openAIKey] }, async (request) => {
    try {
        requireAuth(request.auth);
        const openai = getOpenAI();
        const { text, voice, speed } = request.data;
        ...
        if (!response) {
            throw lastModelError !== null && lastModelError !== void 0 ? lastModelError : new Error('No AI model available for text-to-speech.');
        }
...
exports.genieTranscribeAudio = (0, https_1.onCall)({ secrets: [openAIKey] }, async (request) => {
    var _a;
    requireAuth(request.auth);
    const openai = getOpenAI();
    const { base64Audio, mimeType } = request.data;
    if (!base64Audio) {
        throw new https_1.HttpsError('invalid-argument', 'base64Audio is required.');
    }
    const buffer = Buffer.from(base64Audio, 'base64');
    const extension = ((_a = mimeType === null || mimeType === void 0 ? void 0 : mimeType.split('/')[1]) === null || _a === void 0 ? void 0 : _a.split(';')[0]) || 'webm';
    const file = await (0, openai_1.toFile)(buffer, `audio.${extension}`, { type: mimeType || 'audio/webm' });
...
const sendEmail = async (orgId, to, subject, html) => {
    ...
    mail_1.default.setApiKey(settings.apiKey);
...
exports.genieReportScheduler = (0, scheduler_1.onSchedule)('every 24 hours', async () => {
...
exports.managerDigestScheduler = (0, scheduler_1.onSchedule)('0 7 * * 1', async () => {
    var _a;
    ...
    for (const doc of snapshot.docs) {
        const org = doc.data();
        const notifications = (_a = org.settings) === null || _a === void 0 ? void 0 : _a.notifications;
        if (!(notifications === null || notifications === void 0 ? void 0 : notifications.managerDigestEnabled))
            continue;
...
exports.managerDigestProcessor = (0, scheduler_1.onSchedule)('every 60 minutes', async () => {
        ...
        try {
            ...
            for (const managerDoc of managersSnap.docs) {
                ...
                const topCourses = Array.from(courseCounts.entries())
                    ...
                    .map(([courseId, count]) => {
                    var _a;
                    const completedCount = relevantEnrollments.filter((e) => e.courseId === courseId && e.status === 'completed').length;
                    const completionRate = count > 0 ? Math.round((completedCount / count) * 100) : 0;
                    return {
                        title: ((_a = courseMap.get(courseId)) === null || _a === void 0 ? void 0 : _a.title) || 'Course',
                        count,
                        completionRate
                    };
                });
                ...
                if (!manager.email)
                    continue;
                const reportUrl = (manager === null || manager === void 0 ? void 0 : manager.userId)
                    ? `https://app.tuutta.com/admin/genie/notifications?manager=${manager.userId}`
                    : 'https://app.tuutta.com/admin/genie/notifications';
                await sendEmail(run.orgId, [manager.email], 'Manager Digest', `
...
exports.sendDeadlineReminders = (0, scheduler_1.onSchedule)('0 9 * * *', async () => {
    var _a, _b;
    ...
    for (const orgDoc of orgs.docs) {
        ...
        const notifications = (_a = org.settings) === null || _a === void 0 ? void 0 : _a.notifications;
        if (!(notifications === null || notifications === void 0 ? void 0 : notifications.assignmentDue))
            continue;
        const reminderDays = (_b = notifications.reminderDaysBefore) !== null && _b !== void 0 ? _b : 3;
...
exports.checkOverdueEnrollments = (0, scheduler_1.onSchedule)('0 6 * * *', async () => {
    var _a;
    const orgs = await db.collection('organizations').get();
    if (orgs.empty)
        return;
    ...
    for (const orgDoc of orgs.docs) {
        ...
        for (const enrollmentDoc of overdueSnap.docs) {
            ...
            await db.collection('lmsNotifications').add({
                ...
                channels: [
                    ...((notifications === null || notifications === void 0 ? void 0 : notifications.inAppEnabled) ? ['in_app'] : []),
                    ...((notifications === null || notifications === void 0 ? void 0 : notifications.emailEnabled) ? ['email'] : []),
                    ...((notifications === null || notifications === void 0 ? void 0 : notifications.pushEnabled) ? ['push'] : [])
                ],
                status: 'pending',
                createdAt: Date.now()
            });
...
const selectMembersForRule = async (orgId, rule) => {
    ...
    return members.filter((member) => {
        ...
        if (((_b = (_a = rule.conditions) === null || _a === void 0 ? void 0 : _a.roles) === null || _b === void 0 ? void 0 : _b.length) && !rule.conditions.roles.includes(member.role))
            return false;
        if (((_d = (_c = rule.conditions) === null || _c === void 0 ? void 0 : _c.departments) === null || _d === void 0 ? void 0 : _d.length) && !rule.conditions.departments.includes(member.departmentId))
            return false;
        if (((_f = (_e = rule.conditions) === null || _e === void 0 ? void 0 : _e.teams) === null || _f === void 0 ? void 0 : _f.length) && !rule.conditions.teams.includes(member.teamId))
            return false;
        return true;
    });
};
const applyAssignmentRules = async (orgId, trigger, memberOverride, courseOverride) => {
    var _a, _b, _c;
    ...
    for (const rule of rules) {
        if (courseOverride && !((_a = rule.courseIds) === null || _a === void 0 ? void 0 : _a.includes(courseOverride.id)))
            continue;
        ...
        for (const member of members) {
            for (const courseId of rule.courseIds || []) {
                await ensureEnrollment({
                    ...
                    courseId,
                    learningPathId: (_b = rule.learningPathIds) === null || _b === void 0 ? void 0 : _b[0],
                    assignedBy: 'system',
                    priority: rule.priority || 'required',
                    dueDate: ((_c = rule.dueDate) === null || _c === void 0 ? void 0 : _c.type) === 'fixed' ? rule.dueDate.date : undefined
                });
            }
        }
    }
};
exports.enrollmentCreated = (0, firestore_1.onDocumentCreated)('enrollments/{enrollmentId}', async (event) => {
    var _a, _b;
    if (!event.data)
        return;
    ...
    if (!orgId)
        return;
    const orgSnap = await db.collection('organizations').doc(orgId).get();
    const orgName = orgSnap.exists ? (_a = orgSnap.data()) === null || _a === void 0 ? void 0 : _a.name : 'Tuutta';
    ...
    const managerId = member === null || member === void 0 ? void 0 : member.managerId;
    const courseSnap = await db.collection('courses').doc(enrollment.courseId).get();
    const courseTitle = courseSnap.exists ? (_b = courseSnap.data()) === null || _b === void 0 ? void 0 : _b.title : 'training';
    ...
    if (member === null || member === void 0 ? void 0 : member.email) {
        await sendEmail(orgId, [member.email], `Welcome to ${orgName}`, `
    ...
    if (managerId) {
        ...
        if (manager === null || manager === void 0 ? void 0 : manager.email) {
            await sendEmail(orgId, [manager.email], `Team member assigned training`, `
          <p>${(member === null || member === void 0 ? void 0 : member.name) || 'A learner'} was enrolled in <strong>${courseTitle}</strong>.</p>
        `);
        }
    }
});
exports.enrollmentCreatedOrg = (0, firestore_1.onDocumentCreated)('organizations/{orgId}/enrollments/{enrollmentId}', async (event) => {
    var _a, _b;
    if (!event.data)
        return;
    const enrollment = event.data.data();
...
exports.memberCreated = (0, firestore_1.onDocumentCreated)('orgMembers/{memberId}', async (event) => {
    if (!event.data)
        return;
...
exports.coursePublished = (0, firestore_1.onDocumentUpdated)('courses/{courseId}', async (event) => {
    if (!event.data)
        return;
...
exports.coursePublishedOrg = (0, firestore_1.onDocumentUpdated)('organizations/{orgId}/courses/{courseId}', async (event) => {
    if (!event.data)
        return;
    const beforeData = event.data.before.data() || {};
    const afterData = event.data.after.data() || {};
...
exports.policyUpdated = (0, firestore_1.onDocumentUpdated)({ document: 'organizations/{orgId}', secrets: [openAIKey] }, async (event) => {
    var _a, _b, _c, _d;
    if (!event.data)
        return;
    const before = event.data.before.data() || {};
    const after = event.data.after.data() || {};
    const beforeCompliance = ((_a = before.settings) === null || _a === void 0 ? void 0 : _a.compliance) || {};
    const afterCompliance = ((_b = after.settings) === null || _b === void 0 ? void 0 : _b.compliance) || {};
    if (JSON.stringify(beforeCompliance) === JSON.stringify(afterCompliance))
        return;
    await createAuditLogEntry({
        orgId: event.params.orgId,
        actorId: 'system',
...
exports.competencyRefreshScheduler = (0, scheduler_1.onSchedule)('every monday 06:00', async () => {
    try {
        const orgs = await db.collection('organizations').get();
        if (orgs.empty)
            return;
...
const computeOrgAnalytics = async (orgId) => {
    var _a, _b;
    ...
    let bestF1 = 0;
    const scoreForEnrollment = (enrollment) => {
        const userId = enrollment.userId;
    ...
    for (const enrollment of enrollments) {
        ...
        if (riskLevel !== 'low')
            atRiskEnrollments.push(enrollment.id);
        riskScores.push({
    ...
    const topCourseTitle = topCourse
        ? (((_a = courses.find((c) => c.id === topCourse.courseId)) === null || _a === void 0 ? void 0 : _a.title) || 'recommended course')
        : 'recommended course';
    ...
    for (const risk of riskScores.filter((r) => r.riskLevel === 'high')) {
        ...
        const member = members.find((m) => (m.userId || m.id) === risk.userId);
        const managerId = member === null || member === void 0 ? void 0 : member.managerId;
        if (managerId) {
            ...
            const managerRec = {
                ...
                message: `${(member === null || member === void 0 ? void 0 : member.name) || 'A learner'} is at risk of missing a deadline.`,
                severity: 'high',
                createdAt: now
            };
            ...
            await createNotification({
                ...
                title: 'Team member at risk',
                message: `${(member === null || member === void 0 ? void 0 : member.name) || 'A learner'} is at risk of missing a training deadline.`,
    ...
    for (const result of assessments) {
        const answers = Array.isArray((_b = result.metadata) === null || _b === void 0 ? void 0 : _b.questionResults) ? result.metadata.questionResults : [];
...
exports.analyticsRefreshScheduler = (0, scheduler_1.onSchedule)('every monday 07:00', async () => {
    try {
        const orgs = await db.collection('organizations').get();
        if (orgs.empty)
            return;
...
exports.archiveOldCoursesScheduler = (0, scheduler_1.onSchedule)('0 3 1 * *', async () => {
    try {
...
exports.retentionPolicyScheduler = (0, scheduler_1.onSchedule)('0 4 1 * *', async () => {
    var _a, _b;
    try {
        const orgs = await db.collection('organizations').get();
        for (const orgDoc of orgs.docs) {
            const org = orgDoc.data();
            const policies = ((_b = (_a = org.settings) === null || _a === void 0 ? void 0 : _a.compliance) === null || _b === void 0 ? void 0 : _b.retentionPolicies) || [];
...
const isRegression = (from, to) => {
    ...
    const fromRank = (_a = order[from]) !== null && _a !== void 0 ? _a : 0;
    const toRank = (_b = order[to]) !== null && _b !== void 0 ? _b : 0;
    if (from === 'overdue' && to === 'failed')
        return false;
    return toRank < fromRank;
};
const enforceEnrollmentStatus = async (beforeData, afterData, ref, orgId) => {
...
exports.enrollmentStatusGuard = (0, firestore_1.onDocumentUpdated)('enrollments/{enrollmentId}', async (event) => {
    if (!event.data)
        return;
    const beforeData = event.data.before.data() || {};
    const afterData = event.data.after.data() || {};
...
exports.enrollmentStatusGuardOrg = (0, firestore_1.onDocumentUpdated)('organizations/{orgId}/enrollments/{enrollmentId}', async (event) => {
    if (!event.data)
        return;
    const beforeData = event.data.before.data() || {};
    const afterData = event.data.after.data() || {};
...
exports.assessmentResultGuard = (0, firestore_1.onDocumentUpdated)('assessmentResults/{resultId}', async (event) => {
    if (!event.data)
        return;
    const beforeData = event.data.before.data() || {};
    const afterData = event.data.after.data() || {};
...
exports.assessmentResultGuardOrg = (0, firestore_1.onDocumentUpdated)('organizations/{orgId}/assessments/{resultId}', async (event) => {
    if (!event.data)
        return;
    const beforeData = event.data.before.data() || {};
    const afterData = event.data.after.data() || {};
...
const issueCertificateForEnrollment = async (enrollment, enrollmentId, orgId) => {
    ...
    if (course && course.settings && course.settings.enableCertificate === false)
        return;
    const courseTitle = (course === null || course === void 0 ? void 0 : course.title) || 'Course';
    ...
    const assessmentScores = assessmentSnap.docs.map(doc => doc.data().score || 0);
    ...
    const certificate = {
        ...
        evidence: {
            ...
            issuedBy: 'system',
            assessmentScore,
            courseVersion: course === null || course === void 0 ? void 0 : course.version
        }
    };
...
const issuePathCertificate = async (orgId, userId, learningPathId) => {
    var _a, _b, _c;
    ...
    if (((_a = path.certification) === null || _a === void 0 ? void 0 : _a.enabled) === false)
        return;
    const enrollmentsSnap = await db.collection('enrollments')
        .where('orgId', '==', orgId)
        .where('userId', '==', userId)
    ...
    const verificationUrl = `${baseUrl}/verify/${certNumber}`;
    const qrImageUrl = `https://api.qrserver.com/v1/create-qr-code/?size=160x160&data=${encodeURIComponent(verificationUrl)}`;
    const expiresAt = ((_b = path.certification) === null || _b === void 0 ? void 0 : _b.validityPeriod)
    ...
    const certificate = {
        ...
        userId,
        learningPathId,
        title: ((_c = path.certification) === null || _c === void 0 ? void 0 : _c.title) || `Learning Path Certificate - ${path.title}`,
        issuedAt: Date.now(),
        expiresAt,
        certificateNumber: certNumber,
        verificationCode,
...
exports.enrollmentCompletionIssuer = (0, firestore_1.onDocumentUpdated)('enrollments/{enrollmentId}', async (event) => {
    if (!event.data)
        return;
    const beforeData = event.data.before.data() || {};
    const afterData = event.data.after.data() || {};
...
exports.enrollmentCompletionIssuerOrg = (0, firestore_1.onDocumentUpdated)('organizations/{orgId}/enrollments/{enrollmentId}', async (event) => {
    if (!event.data)
        return;
    const beforeData = event.data.before.data() || {};
    const afterData = event.data.after.data() || {};
...
const handleAssessmentResultCreated = async (data, orgIdOverride) => {
    var _a;
    ...
    if (!passed && attempts >= 2) {
        ...
        if (managerId) {
            await sendRemediationNotification({
                ...
                message: `${(member === null || member === void 0 ? void 0 : member.name) || 'A learner'} has failed an assessment multiple times.`,
    ...
    if (passed && metadata.assessmentRole === 'module' && metadata.moduleId) {
        ...
        if (courseId && metadata.moduleId) {
            ...
            const course = courseSnap.exists ? courseSnap.data() : null;
            const module = (_a = course === null || course === void 0 ? void 0 : course.modules) === null || _a === void 0 ? void 0 : _a.find((item) => item.id === metadata.moduleId);
            if (module === null || module === void 0 ? void 0 : module.title)
...
exports.assessmentResultCreated = (0, firestore_1.onDocumentCreated)('assessmentResults/{resultId}', async (event) => {
    if (!event.data)
        return;
    await handleAssessmentResultCreated(event.data.data());
});
exports.assessmentResultCreatedOrg = (0, firestore_1.onDocumentCreated)('organizations/{orgId}/assessments/{resultId}', async (event) => {
    if (!event.data)
        return;
    await handleAssessmentResultCreated(event.data.data(), event.params.orgId);
});
exports.notificationDispatcher = (0, scheduler_1.onSchedule)('every 5 minutes', async () => {
...
exports.genieEvidenceExport = (0, https_1.onCall)(async (request) => {
    var _a, _b, _c, _d, _e;
    requireAuth(request.auth);
    const orgId = String(((_a = request.data) === null || _a === void 0 ? void 0 : _a.orgId) || '');
    if (!orgId) {
        throw new https_1.HttpsError('invalid-argument', 'orgId is required.');
    }
    const memberDoc = await db.collection('orgMembers').doc(`${orgId}_${(_b = request.auth) === null || _b === void 0 ? void 0 : _b.uid}`).get();
    if (!memberDoc.exists) {
        throw new https_1.HttpsError('permission-denied', 'Org membership required.');
    }
    const startRaw = ((_c = request.data) === null || _c === void 0 ? void 0 : _c.startDate) ? Number(request.data.startDate) : null;
    const endRaw = ((_d = request.data) === null || _d === void 0 ? void 0 : _d.endDate) ? Number(request.data.endDate) : null;
    ...
    const orgSnap = await db.collection('organizations').doc(orgId).get();
    const orgName = orgSnap.exists ? (_e = orgSnap.data()) === null || _e === void 0 ? void 0 : _e.name : 'Organization';
    ...
    const trainingCompletionData = enrollments
        ...
        return {
            learner: (member === null || member === void 0 ? void 0 : member.name) || 'Learner',
            email: (member === null || member === void 0 ? void 0 : member.email) || '-',
            course: (course === null || course === void 0 ? void 0 : course.title) || enrollment.courseId,
    ...
    const assessmentData = assessmentResults.map((result) => {
        ...
        return {
            learner: (member === null || member === void 0 ? void 0 : member.name) || 'Learner',
            email: (member === null || member === void 0 ? void 0 : member.email) || '-',
            course: (course === null || course === void 0 ? void 0 : course.title) || result.courseId,
    ...
    for (const cert of certificates) {
        ...
        const pdfBuffer = await createCertificatePdfBuffer({
            orgName,
            learnerName: (member === null || member === void 0 ? void 0 : member.name) || 'Learner',
            courseTitle: (course === null || course === void 0 ? void 0 : course.title) || cert.courseId || 'Course',
...
exports.testOrgWebhook = (0, https_1.onCall)(async (request) => {
    var _a;
    requireAuth(request.auth);
    const orgId = String(((_a = request.data) === null || _a === void 0 ? void 0 : _a.orgId) || '');
    if (!orgId) {
...
exports.recalculateAnalytics = (0, https_1.onCall)(async (request) => {
    var _a;
    requireAuth(request.auth);
    const orgId = String(((_a = request.data) === null || _a === void 0 ? void 0 : _a.orgId) || '');
    if (!orgId) {
    ...
    try {
        ...
        throw new https_1.HttpsError('internal', 'Analytics recalculation failed.');
    }
});
